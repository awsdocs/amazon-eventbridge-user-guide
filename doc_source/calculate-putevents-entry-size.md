# Calculating PutEvents Event Entry Sizes<a name="calculate-putevents-entry-size"></a>

You can inject custom events into EventBridge using the `PutEvents` action\. You can inject multiple events using the `PutEvents` action as long as the total entry size is less than 256KB\. You can calculate the event entry size beforehand by following the steps below\. You can then batch multiple event entries into one request for efficiency\.
